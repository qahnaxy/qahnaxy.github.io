document.addEventListener("DOMContentLoaded", () => {
  ...
  function createParticle(type, color, size) {
    ...
    particle.className =
      `contact-me-particle contact-me-particle--${type}`;
    ...
    const frames = [
      {
        opacity: 0,
        transform:
          "translate(-50%, -50%) translate(0px, 10px) scale(0.25) rotate(0deg)"
      },
      {
        offset: 0.16,
        opacity: 1,
        transform:
          `translate(-50%, -50%) translate(${x * 0.42}px, ${y * 0.42}px) scale(${scale}) rotate(${rotation * -0.55}deg)`
      },
      {
        offset: 0.34,
        opacity: 1,
        transform:
          `translate(-50%, -50%) translate(${x * 0.74}px, ${y * 0.74}px) scale(${scale * 1.04}) rotate(${rotation}deg)`
      },
      {
        offset: 0.46,
        opacity: 0.98,
        transform:
          `translate(-50%, -50%) translate(${x * 0.74}px, ${y * 0.74}px) scale(${scale * 1.02}) rotate(${rotation * -0.85}deg)`
      },
      {
        offset: 0.64,
        opacity: 0.9,
        transform:
          `translate(-50%, -50%) translate(${x}px, ${y}px) scale(${scale}) rotate(${rotation * 0.5}deg)`
      },
      {
        opacity: 0,
        transform:
          `translate(-50%, -50%) translate(${x * 0.14}px, ${finalYOffset}px) scale(0.3) rotate(${rotation * 1.35}deg)`
      }
    ];
  ...
  function triggerBurst(wrapper) {
    ...
    const burstLayer = wrapper.querySelector(
      "[data-contact-animation-burst]"
    );
  ...
  wrappers.forEach((wrapper) => {
    const trigger = wrapper.querySelector(
      "[data-contact-animation-trigger]"
    );
